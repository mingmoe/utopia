...
def call_ninja(args: List[str], stdout_file: str, stderr_file: str, work_dir: str) -> None:
    with open(stdout_file, "a", encoding='utf-8') as stdout, \
        open(stderr_file, "a", encoding='utf-8') as stderr:
...
class CMakeTask:
    ...
    def run_once(self, options: [str]):
        with open(self.stdout_file, "a", encoding='utf-8') as stdout, \
            open(self.stderr_file, "a", encoding='utf-8') as stderr:
...
build_library('libpng', ["-DPNG_SHARED=OFF", "-DPNG_STATIC=ON", "-DPNG_TEST=OFF",
                         f"-DZLIB_LIBRARY={get_install_path_of_library('zlib')}/lib/zlibstatic.lib",
...
build_library('freetype', [
    ...
    f"-DPNG_LIBRARY={get_install_path_of_library('libpng')}/lib/libpng16_static.lib"])
